...
int best_fit(Particion particiones[], int n, int tam_proceso) {
    int mejor_indice = -1;
    int mejor_tamano = 99999999; /* Inicializado con un valor muy grande */
    int i;
...
int worst_fit(Particion particiones[], int n, int tam_proceso) {
    int mejor_indice = -1;
    int mejor_tamano = -1; /* Inicializado con un valor pequeno */
    int i;
...
int next_fit(Particion particiones[], int n, int tam_proceso) {
    ...
    for (contador = 0; contador < n; contador++) {
        ...
        if (particiones[indice].estado == LIBRE && particiones[indice].tamano >= tam_proceso) {
            puntero_next_fit = (indice + 1) % n; /* Actualiza para la siguiente busqueda */
            return indice;
        }
    }
    return -1; 
